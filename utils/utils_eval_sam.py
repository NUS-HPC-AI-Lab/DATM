...
def epoch(mode, dataloader, net, optimizer, scheduler,epoch, criterion, args, aug, texture=False):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False):
    ...
    optimizer = SAM(net.parameters(), base_optim,lr=lr, momentum=0.9, weight_decay=0.0005,rho=0.05)
    scheduler = StepLR(optimizer, lr, Epoch)
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        loss_train, acc_train = epoch('train', trainloader, net, optimizer, scheduler, ep, criterion, args, aug=True, texture=texture)
        acc_train_list.append(acc_train)
        loss_train_list.append(loss_train)
        if ep == Epoch:
            with torch.no_grad():
                loss_test, acc_test = epoch('test', testloader, net, optimizer,scheduler, ep, criterion, args, aug=False)
