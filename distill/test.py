...
def main(args):
    ...
    # elif args.pix_init == 'mix_up':
    #     print('initialize synthetic data from random mix up')
    #     temp = None
    #     for c in range(num_classes):
    #         # image_syn.data[c * args.ipc:(c + 1) * args.ipc] = (get_images(c, args.ipc).detach().data + get_images((c+1)%num_classes, args.ipc).detach().data)/2.
            
    #         if temp == None:
    #             temp = get_images(c, args.ipc * 2).detach().data
    #         else:
    #             temp = torch.cat((temp, get_images(c, args.ipc * 2).detach().data), dim=0)
    #     _index = [i for i in range(len(temp))]
    #     random.shuffle(_index)
    #     _index = torch.tensor(_index)
    #     image_syn.data[0:len(image_syn)] = (torch.index_select(temp, 0, _index[0:len(image_syn)]) + torch.index_select(temp, 0, _index[len(image_syn):len(_index)]))/2.
            
            # combine 4 pic of the same class
            # temp = get_images(c, args.ipc * 4).detach().data
            # n, _, w, h = temp.shape
            # temp = F.interpolate(temp, scale_factor=0.5, mode='bilinear')
            # TEMP_1 = torch.cat((temp[0:args.ipc], temp[args.ipc:args.ipc*2]), dim=3)
            # TEMP_2 = torch.cat((temp[args.ipc*2:args.ipc*3], temp[args.ipc*3:args.ipc*4]), dim=3)
            # image_syn.data[c * args.ipc:(c + 1) * args.ipc] = torch.cat((TEMP_1, TEMP_2), dim=2)
            
            
    # test
    # elif args.pix_init == 'mixture':
    #     print('initialize synthetic data as the mixture of real images')
    #     for c in range(num_classes):
    #         temp = get_images(c, len(indices_class[c])).detach().data
    #         mix_num = len(temp)/args.ipc
    #         temp = torch.stack(torch.split(temp, int(mix_num), dim=0))
    #         image_syn.data[c * args.ipc:(c + 1) * args.ipc] = torch.sum(temp, dim=1)/mix_num
    # elif args.pix_init == 'mixture_kmeans':
    #     print('initialize synthetic data as the mixture of real images divided by kemeans')
    #     for c in range(num_classes):
    #         data_for_class_c = get_images(c, len(indices_class[c])).detach().data
    #         n, _, w, h = data_for_class_c.shape
    #         cluster_ids_x, cluster_centers = kmeans(
    #             X=data_for_class_c.view(n, -1), num_clusters=args.ipc, distance='euclidean', device=torch.device('cuda:0')
    #         )
    #         store=[[] for i in range(args.ipc)]
    #         # index of data contained in each cluster
    #         for i in range(len(cluster_ids_x)):
    #             store[cluster_ids_x[i]].append(i)
    #         temp_img = None
    #         for nu in range(len(store)):
    #             temp = torch.index_select(data_for_class_c, dim=0, index=torch.tensor(store[nu]))
    #             # temp = (torch.sum(temp, dim=0)/len(temp)).unsqueeze(dim=0)
    #             metric = torch.sum((temp-cluster_centers[nu].view(_,w,h)).pow(2), dim=(1,2,3))
    #             temp=temp[metric.argmin()].unsqueeze(dim=0)
                
    #             if temp_img is None:
    #                 temp_img = temp
    #             else:
    #                 temp_img = torch.cat((temp_img, temp), dim=0)
    #         image_syn.data[c * args.ipc:(c + 1) * args.ipc] = temp_img
    #         # image_syn.data[c * args.ipc:(c + 1) * args.ipc] = cluster_centers.view(args.ipc,3,w,h)
    # elif args.pix_init == 'samples_near_center':
    #     for c in range(num_classes):
    #         data_for_class_c = get_images(c, len(indices_class[c])).detach().data
    #         n, _, w, h = data_for_class_c.shape
    #         cluster_centers = torch.sum(data_for_class_c, dim=0)/n

    #         # temp = (torch.sum(temp, dim=0)/len(temp)).unsqueeze(dim=0)
    #         metric = torch.sum((data_for_class_c-cluster_centers).pow(2), dim=(1,2,3))
    #         sorted, indices = torch.sort(metric, descending=True)
    #         temp = torch.index_select(data_for_class_c, dim=0, index=indices[:args.ipc])
    #         image_syn.data[c * args.ipc:(c + 1) * args.ipc] = temp
    #         # image_syn.data[c * args.ipc:(c + 1) * args.ipc] = cluster_centers.view(args.ipc,3,w,h)
        ...
        if cfg.Initialize_Label_With_Another_Model:
            label_expert_files = []
            label_expert_dir = os.path.join(args.buffer_path, args.dataset)
            if args.dataset == "ImageNet":
                label_expert_dir = os.path.join(label_expert_dir, args.subset, str(args.res))
            if args.dataset in ["CIFAR10", "CIFAR100"] and not args.zca:
                label_expert_dir += "_NO_ZCA"
            label_expert_dir = os.path.join(label_expert_dir, args.Initialize_Label_Model)
            n = 0
            while os.path.exists(os.path.join(label_expert_dir, "replay_buffer_{}.pt".format(n))):
                label_expert_files.append(os.path.join(label_expert_dir, "replay_buffer_{}.pt".format(n)))
                n += 1
            if n == 0:
                raise AssertionError("No buffers detected at {}".format(expert_dir))
        else:
    ...
    ema_label_best_acc = {m: 0 for m in model_eval_pool}
    ema_label_best_std = {m: 0 for m in model_eval_pool}
    ...
    # label_syn = torch.nn.functional.one_hot(label_syn, num_classes).float().to(args.device)
    ...
    ema_label = ModelEmaV2([label_syn], decay=args.ema_decay)
    ...
    for it in range(0, args.Iteration+1):
        ...
        if it in eval_it_pool:
            ...
            if args.record_loss:
                if args.parall_eva==False:
                    ...
                    device = args.device
                student_net = get_network(args.model, channel, num_classes, im_size, dist=False).to(device)  # get a random model
                ...
                for i in range(log_length):
                    ...
                    syn_images = image_syn.detach()
                    y_hat = label_syn.detach().to(args.device)
                    param_loss_list = []
                    param_dist_list = []
                    indices_chunks = []
            ...
            for model_eval in model_eval_pool:
                ...
                ''' Evaluate ema synthetic data and ema synthetic label '''
                ema_label_accs_test = []
                ema_label_accs_train = []
                ...
                for it_eval in range(args.num_eval):
                    ...
                    ''' Evaluate ema synthetic data '''
                    ema_eval_labs = label_syn.detach()
                    with torch.no_grad():
                        ema_image_syn = ema.module[0]
                        ema_image_save = ema_image_syn
                    ema_image_syn_eval, ema_syn_eval = copy.deepcopy(ema_image_save), copy.deepcopy(ema_eval_labs.detach()) # avoid any unaware modification
                    _, ema_acc_train, ema_acc_test = evaluate_synset(it_eval, copy.deepcopy(net_eval), ema_image_syn_eval, ema_syn_eval, testloader, args, texture=False, train_criterion=criterion)
                    ema_accs_test.append(ema_acc_test)
                    ema_accs_train.append(ema_acc_train)

                    ''' Evaluate ema synthetic data and ema synthetic label '''
                    # with torch.no_grad():
                    #     ema_eval_labs = ema_label.module[0]
                    #     ema_image_syn = ema.module[0]
                    #     ema_image_save = ema_image_syn
                    ...
                    # _, ema_label_acc_train, ema_label_acc_test = evaluate_synset(it_eval, copy.deepcopy(net_eval), ema_image_syn_eval, ema_label_syn_eval, testloader, args, texture=False, train_criterion=criterion)
                    # ema_label_accs_test.append(ema_label_acc_test)
                    # ema_label_accs_train.append(ema_label_acc_train)
                ...
                ''' Evaluate ema synthetic data and ema synthetic label'''
                # ema_label_accs_test = np.array(ema_label_accs_test)
                # ema_label_accs_train = np.array(ema_label_accs_train)
                # ema_label_acc_test_mean = np.mean(ema_label_accs_test)
                # ema_label_acc_test_std = np.std(ema_label_accs_test)
                
                # if ema_label_acc_test_mean > ema_label_best_acc[model_eval]:
                #     ema_label_best_acc[model_eval] = ema_label_acc_test_mean
                #     ema_label_best_std[model_eval] = ema_label_acc_test_std
                #     save_this_it = True
                ...
                # ''' Evaluate EMA synthetic data and EMA synthetic label '''
                # print('Evaluate EMA %d random %s, EMA_mean = %.4f EMA_std = %.4f\n-------------------------'%(len(ema_label_accs_test), model_eval, ema_label_acc_test_mean, ema_label_acc_test_std))
                # wandb.log({'EMA_Accuracy/{}'.format(model_eval): ema_label_acc_test_mean}, step=it)
                # wandb.log({'EMA_Max_Accuracy/{}'.format(model_eval): ema_label_best_acc[model_eval]}, step=it)
                # wandb.log({'EMA_Std/{}'.format(model_eval): ema_label_acc_test_std}, step=it)
                # wandb.log({'EMA_Max_Std/{}'.format(model_eval): ema_label_best_std[model_eval]}, step=it)
            ...
            with torch.no_grad():
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    if args.zca:
                        ...
                        torch.save(ema_image_save.cpu(), os.path.join(save_dir, 'ema', "ema_images_zca_{}.pt".format(it)))
        ...
        # test
        Upper_Bound = args.Initial_Step + int((args.max_start_epoch-args.Initial_Step) * it/(args.learn_end_epoch))
        Upper_Bound = min(Upper_Bound, args.max_start_epoch)

        start_epoch = np.random.randint(args.min_start_epoch, Upper_Bound)

        # test
        if start_epoch > curMax:
            start_epoch = curMax
            curMax = min(Upper_Bound, curMax+1)
